...
int main()
{
    ...
    // To find a substring in a string 'str' (str::npos is returned if not found).
    ...
    // If the string passed in the previous 'cin' function has a whitespace in between, cin will read only the part before the whitespace and the rest will be read by getline (given that they are written consecutively), that's why the first getline here is not accepting input.
    string name2;
    getline(cin, name2);
    cout<<name2<<"\n";
    getline(cin, name2, 'i');
    ...
    // In case of the pattern (size m) having all characters as distinct, irrespective of the text (size n), we can perform the pattern searching problem in O(n) as you can skip the part that did match.
